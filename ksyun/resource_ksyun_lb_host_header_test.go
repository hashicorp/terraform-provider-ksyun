package ksyun

import (
	"fmt"
	"github.com/hashicorp/terraform-plugin-sdk/helper/resource"
	"github.com/hashicorp/terraform-plugin-sdk/terraform"
	"testing"
)

func TestAccKsyunListenerHostHeader_basic(t *testing.T) {
	var val map[string]interface{}
	resource.ParallelTest(t, resource.TestCase{
		PreCheck: func() {
			testAccPreCheck(t)
		},
		IDRefreshName: "ksyun_listener_host_header.foo",
		Providers:     testAccProviders,
		CheckDestroy:  testAccCheckListenerHostHeaderDestroy,
		Steps: []resource.TestStep{
			{
				Config: testAccListenerHostHeaderConfig,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckListenerHostHeaderExists("ksyun_listener_host_header.foo", &val),
					testAccCheckListenerHostHeaderAttributes(&val),
				),
			},
		},
	})
}

func TestAccKsyunListenerHostHeader_update(t *testing.T) {
	var val map[string]interface{}
	resource.ParallelTest(t, resource.TestCase{
		PreCheck: func() {
			testAccPreCheck(t)
		},
		IDRefreshName: "ksyun_listener_host_header.foo",
		Providers:     testAccProviders,
		CheckDestroy:  testAccCheckListenerHostHeaderDestroy,
		Steps: []resource.TestStep{
			{
				Config: testAccListenerHostHeaderConfig,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckListenerHostHeaderExists("ksyun_listener_host_header.foo", &val),
					testAccCheckListenerHostHeaderAttributes(&val),
				),
			},
			{
				Config: testAccListenerHostHeaderUpdateConfig,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckListenerHostHeaderExists("ksyun_listener_host_header.foo", &val),
					testAccCheckListenerHostHeaderAttributes(&val),
				),
			},
		},
	})
}

func testAccCheckListenerHostHeaderExists(n string, val *map[string]interface{}) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		rs, ok := s.RootModule().Resources[n]
		if !ok {
			return fmt.Errorf("not found: %s", n)
		}
		if rs.Primary.ID == "" {
			return fmt.Errorf("Host header id is empty")
		}
		client := testAccProvider.Meta().(*KsyunClient)
		hostHeader := make(map[string]interface{})
		hostHeader["HostHeaderId.1"] = rs.Primary.ID
		ptr, err := client.slbconn.DescribeHostHeaders(&hostHeader)
		if err != nil {
			return err
		}
		if ptr != nil {
			l := (*ptr)["HostHeaderSet"].([]interface{})
			if len(l) == 0 {
				return err
			}
		}

		*val = *ptr
		return nil
	}
}
func testAccCheckListenerHostHeaderAttributes(val *map[string]interface{}) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		if val != nil {
			l := (*val)["HostHeaderSet"].([]interface{})
			if len(l) == 0 {
				return fmt.Errorf("host header id is empty")
			}
		}
		return nil
	}
}
func testAccCheckListenerHostHeaderDestroy(s *terraform.State) error {
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "ksyun_listener_host_header" {
			continue
		}

		client := testAccProvider.Meta().(*KsyunClient)
		hostHeader := make(map[string]interface{})
		hostHeader["HostHeaderId.1"] = rs.Primary.ID
		ptr, err := client.slbconn.DescribeHostHeaders(&hostHeader)

		// Verify the error is what we want
		if err != nil {
			return err
		}
		if ptr != nil {
			l := (*ptr)["HostHeaderSet"].([]interface{})
			if len(l) == 0 {
				continue
			} else {
				return fmt.Errorf("host header still exist")
			}
		}
	}

	return nil
}

const testAccListenerHostHeaderConfig = `
resource "ksyun_vpc" "default" {
  vpc_name        = "tf-acc-vpc"
  cidr_block = "192.168.0.0/16"
}
resource "ksyun_lb" "default" {
  vpc_id = "${ksyun_vpc.default.id}"
  load_balancer_name = "tf-xun-2"
  type = "public"
  subnet_id = ""
  load_balancer_state = "stop"
  private_ip_address = ""
}
# Create Load Balancer Listener with tcp protocol
resource "ksyun_lb_listener" "default" {
  listener_name = "tf-xun-2"
  listener_port = "8080"
  listener_protocol = "HTTPS"
  listener_state = "stop"
  load_balancer_id = "${ksyun_lb.default.id}"
  method = "RoundRobin"
  certificate_id = "${ksyun_certificate.default.id}"
  session {
    session_state = "stop"
    session_persistence_period = 100
    cookie_type = "RewriteCookie"
    cookie_name = "cookiexunqq"
  }
}
resource "ksyun_certificate" "default" {
  certificate_name="certificate1"
  private_key="-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----"
  public_key="-----BEGIN CERTIFICATE-----\\nWYdRxC4CjnSn/uVleWJBFcR0gj6vBPTWhQ4V\n-----END CERTIFICATE-----\n\n-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n\n-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----"
}
resource "ksyun_listener_host_header" "foo" {
  listener_id = "${ksyun_lb_listener.default.id}"
  host_header = "tf-xuan"
  certificate_id = "${ksyun_certificate.default.id}"
}

`

const testAccListenerHostHeaderUpdateConfig = `
resource "ksyun_vpc" "default" {
  vpc_name        = "tf-acc-vpc"
  cidr_block = "192.168.0.0/16"
}
resource "ksyun_lb" "default" {
  vpc_id = "${ksyun_vpc.default.id}"
  load_balancer_name = "tf-xun-2"
  type = "public"
  subnet_id = ""
  load_balancer_state = "stop"
  private_ip_address = ""
}
# Create Load Balancer Listener with tcp protocol
resource "ksyun_lb_listener" "default" {
  listener_name = "tf-xun-2"
  listener_port = "8080"
  listener_protocol = "HTTPS"
  listener_state = "stop"
  load_balancer_id = "${ksyun_lb.default.id}"
  method = "RoundRobin"
  certificate_id = "${ksyun_certificate.default2.id}"
  session {
    session_state = "stop"
    session_persistence_period = 100
    cookie_type = "RewriteCookie"
    cookie_name = "cookiexunqq"
  }
}
resource "ksyun_certificate" "default2" {
  certificate_name="certificate2"
  private_key="-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----"
  public_key="-----BEGIN CERTIFICATE-----\\nWYdRxC4CjnSn/uVleWJBFcR0gj6vBPTWhQ4V\n-----END CERTIFICATE-----\n\n-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n\n-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----"
}

resource "ksyun_listener_host_header" "foo" {
  listener_id = "${ksyun_lb_listener.default.id}"
  host_header = "tf-xuan"
  certificate_id = "${ksyun_certificate.default2.id}"
}
`
